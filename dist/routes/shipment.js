...
const router = Router();
router.get("/", authMiddleware(["admin", "dispatcher", "driver"]), asyncHandler(async (req, res) => {
...
router.get("/:id", authMiddleware(["admin", "dispatcher", "driver"]), asyncHandler(async (req, res) => {
...
router.put("/:id", authMiddleware(["admin", "dispatcher"]), asyncHandler(async (req, res) => {
    ...
    if (status) {
        const allowedStatus = ["pending", "in_transit", "delivered", "cancelled"];
